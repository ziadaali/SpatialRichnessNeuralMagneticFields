...
while ibatch<=Nbatch
    ...
    if ops.spkThPos > 0
        fprintf("Triggered positive spike detection with threshold: %f", ops.spkThPos);
        maxdat = my_min(datr*-1, 30, 1);
        ind = find(((datr<mdat+1e-3) & (datr<ops.spkTh)) | ((datr>-1*(maxdat+1e-3)) & (datr>ops.spkThPos)));
    end
%     if ibatch==1
%         fprintf("Ind2 : %f ", ind);
%         save('batch_vars', 'buff', 'datr', 'mdat', 'ind')
%     end
