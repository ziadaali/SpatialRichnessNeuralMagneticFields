...
if ops.spkThPos > 0
    fprintf("isolated_peaks positive spike detection with threshold: %f", ops.spkThPos);
    smax = my_min(-1*S1, loc_range, [1 2]);
    peaks = single(((S1<smin+1e-3) & (S1<Th)) | ((S1>-1*(smax+1e-3)) & (S1>ops.spkThPos)));
end
