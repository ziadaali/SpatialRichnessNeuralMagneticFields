...
from MEArecTemplate.tools import (select_templates, find_overlapping_templates, get_binary_cat,
...
from MEArecTemplate.generators import SpikeTrainGenerator
                ...
                if 'magnetic_dim' in rec_dict.keys():
                    if rec_dict['magnetic_dim'] != -1:
                        ...
                        if type(mag_dim) == list:
                            ...
                            mag_dim = mag_dim[0]
                        print("Switching recordings in RecordingGenerator to be magnetic recordings with magnetic_dim: ", mag_dim)
    ...
    def generate_recordings(self, tmp_mode=None, tmp_folder=None, verbose=None, template_ids=None, n_jobs=0, magnetic=False):
        ...
        if only_noise:
            ...
            if verbose_1:
                ...
                print("Spiketrains shape: ", np.shape(spiketrains))
        ...
        if noise_level == 0:
            ...
            if magnetic == True:
                ...
                peak_elec_power = np.sum(recordings**2, axis=0)/len(recordings)
                peak_elec_power_idx = np.argmax(peak_elec_power)
                print("Peak Elec Power Index: ", peak_elec_power_idx)
                print("Peak Elec Power: ", peak_elec_power[peak_elec_power_idx])
                #print("Peak Elec Power: ", peak_elec_power)
                ...
                for mag_dim in [0, 1, 2]:
                    ...
                    print("Peak Mag Power shape: ", np.shape(peak_mag_power))
                    #peak_mag_power_idx = np.unravel_index(np.argmax(peak_mag_power), np.shape(mag_recordings_single_dim[0]))
                    ...
                    # Value to scale magnetic signal by to have equal SNR to electric signal at peak electrode
                    mag_scale_factor_dim = np.sqrt(peak_elec_power[peak_elec_power_idx]/peak_mag_power[peak_mag_power_idx])
                    #print("Peak Mag Power: ", peak_mag_power)
                    ...
                    if noise_mode == "ziad-far-neurons":
                        ...
                        mag_recordings[:, mag_dim] *= mag_scale_factor_dim
                        mag_recordings[:, mag_dim] += additive_noise_mag[:, mag_dim]
                        mag_recordings[:, mag_dim] /= mag_scale_factor_dim
        ...
        if magnetic == True:
            ...
            if elecs_row**2 == np.shape(recordings)[-1]:
                # Generate Ey
                print("Elecs_row: ", elecs_row, ", total elecs: ", np.shape(recordings)[-1])
                ...
                new_mag = np.zeros((np.shape(mag_recordings)[0], 5, np.shape(mag_recordings)[2]))
                ...
                print("Not a square grid - skipping Ex, Ey step")
