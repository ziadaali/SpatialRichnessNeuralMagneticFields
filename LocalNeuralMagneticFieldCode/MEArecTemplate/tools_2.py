...
def load_recordings(recordings, return_h5_objects=True,
                    load=None, load_waveforms=True, check_suffix=True, verbose=False, magnetic_dim = -1):
    """
    ...
    if (recordings.suffix in ['.h5', '.hdf5']) or (not check_suffix):
        ...
        if f.get('recordings') is not None and 'recordings' in load:
            ...
            if f.get('mag_recordings') is not None:
                ...
                rec_dict['magnetic_dim'] = magnetic_dim
            if f.get('mag_scale_factor') is not None:
                rec_dict['mag_scale_factor'] = np.max(f.get('mag_scale_factor'))
                print("MEArec tools mag_scale_factor: ", rec_dict['mag_scale_factor'])
