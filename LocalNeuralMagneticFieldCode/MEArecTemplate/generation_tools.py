import MEArecTemplate
from MEArecTemplate.tools import load_templates
from MEArecTemplate.generators import RecordingGenerator, SpikeTrainGenerator, TemplateGenerator
import yaml
import os
...
def gen_recordings(params=None, templates=None, tempgen=None, spgen=None, verbose=True,
                   tmp_mode='memmap', template_ids=None, tmp_folder=None, n_jobs=0, magnetic=False,
                   noise_tempgen=None, noise_templates=None):
    """
    ...
    if noise_tempgen is None:
        if noise_templates is not None:
            noise_templates = Path(noise_templates)
            if noise_templates.is_dir():
                noise_tempgen = load_templates(noise_templates, verbose=False)
            elif noise_templates.suffix in ['.h5', '.hdf5']:
                noise_tempgen = load_templates(noise_templates, verbose=False)
    # else:
    #     raise AttributeError("'noise_templates' is not a folder or an hdf5 file")
    ...
    # Generate recordings
    recgen = RecordingGenerator(spgen, tempgen, params_dict, noise_tempgen=noise_tempgen)
    recgen.generate_recordings(tmp_mode=tmp_mode, tmp_folder=tmp_folder, template_ids=template_ids,
                               n_jobs=n_jobs, verbose=verbose, magnetic=magnetic)
