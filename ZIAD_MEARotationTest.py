...
fine_steps = int(sys.argv[5])
coarse_steps = int(sys.argv[6])
fine_bound_l = float(sys.argv[7])
fine_bound_u = float(sys.argv[8])
coarse_bound_l = float(sys.argv[9])
coarse_bound_u = float(sys.argv[10])

xcoarse = np.linspace(coarse_bound_l, coarse_bound_u, coarse_steps)
ycoarse = np.linspace(coarse_bound_l, coarse_bound_u, coarse_steps)

xfine = np.linspace(fine_bound_l, fine_bound_u, fine_steps)
yfine = np.linspace(fine_bound_l, fine_bound_u, fine_steps)

trans_coords = []
# Add coarse coordinates
for xval in xcoarse:
    for yval in ycoarse:
        if fine_bound_l <= xval <= fine_bound_u and fine_bound_l <= yval <= fine_bound_u:
            pass
        else:
            trans_coords.append([xval, yval])
            
# Add fine coordaintes
for xval in xfine:
    for yval in yfine:
        trans_coords.append([xval, yval])

print("Length trans_coords: ", len(trans_coords))

#x_trans = np.linspace(int(sys.argv[7]), int(sys.argv[8]), x_steps)
#y_trans = np.linspace(int(sys.argv[9]), int(sys.argv[10]), y_steps)

v_corrs = np.zeros((len(thetas), len(trans_coords)))
b_corrs = np.zeros((len(thetas), len(trans_coords)))
...
for n, theta in enumerate(thetas):
    ...
    print("Theta: ", theta)
    '''
    for k, trans_coord in enumerate(trans_coords):
