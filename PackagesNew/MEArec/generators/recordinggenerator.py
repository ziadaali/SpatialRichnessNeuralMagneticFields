                ...
                if 'magnetic_dim' in rec_dict.keys():
                    if rec_dict['magnetic_dim'] != -1:
                        ...
                        if type(mag_dim) == list:
                            print("Mag dim was list: ", mag_dim)
                            convert_negative = mag_dim[1]
                            mag_dim = mag_dim[0]
                        # Add random signals to magnetic recordings in case they are specified in mag_dim by the index 10
                        if isinstance(mag_dim, list):
                            print("Mag_recordings shape before fake signals: ", np.shape(self.mag_recordings))
                            for i in range(mag_dim.count(10)):

                                #Genrate gaussian noise
                                # sigma = np.std(self.mag_recordings[:, 0])
                                # mu = np.mean(self.mag_recordings[:, 0])
                                # fake_sig = np.random.randn(np.shape(self.mag_recordings[:, 0])[0], np.shape(self.mag_recordings[:, 0])[1])
                                # fake_sig = sigma*fake_sig + mu

                                #Generate shuffled noise (same distribution but spikes are now meaningless)
                                permute_length = np.shape(self.mag_recordings[:, 0])[0]*np.shape(self.mag_recordings[:, 0])[1]
                                permute = np.random.permutation(permute_length)
                                fake_sig = np.reshape(self.mag_recordings[:, 0], (permute_length, -1))
                                fake_sig = fake_sig[permute]
                                fake_sig = np.reshape(fake_sig, np.shape(self.mag_recordings[:, 0]))
                                self.mag_recordings = np.concatenate([self.mag_recordings, fake_sig[:, None, :]], axis=1)
                            print("Mag_recordings shape after fake signals: ", np.shape(self.mag_recordings))

                        print("MEArec - Switching recordings in RecordingGenerator to be magnetic recordings with magnetic_dim: ", mag_dim)
                        if isinstance(mag_dim, list):

                            mag_dim_start = 0
                            mag_dim_end = len(mag_dim)
                            if mag_dim[0] == -1:
                                mag_dim_start = 1
                            if mag_dim[-1] >= 99:
                                mag_dim_end -= 1
                            temp_rec = np.copy(self.recordings)
                            self.recordings = self.mag_recordings[:, mag_dim[mag_dim_start:mag_dim_end]]

                            #mag_dim_start = 0
                            #if mag_dim[0] == -1:
                            #    mag_dim_start = 1
                            #temp_rec = np.copy(self.recordings)
                            #self.recordings = self.mag_recordings[:, mag_dim[mag_dim_start:]]
                            if mag_dim_start == 1:
                                self.recordings = np.concatenate([temp_rec[:, None, :], self.recordings], axis=1)
                                print("Max signal in magnetic dimensions: ", np.max(np.abs(self.recordings[:, 1:, :])))
                                print("Max signal in first (voltage) dimensions: ", np.max(np.abs(self.recordings[:, 0, :])))
                        else:
                        ...
                        if 'mag_scale_factor' in rec_dict.keys():
                            ...
                            if isinstance(mag_dim, list):
                                print("Multiplying by scale factor. recordings shape: ", np.shape(self.recordings))
                                print("All scale_factors: ", np.array(self.mag_scale_factor))
                                print("Chosen dimensions: ", mag_dim)
                                for i in range(len(mag_dim)):
                                    if mag_dim[i] < 3 and mag_dim[i] >= 0:
                                        print("scale_factor for dimension ", mag_dim[i], ": ", self.mag_scale_factor[mag_dim[i]])
                                        self.recordings[:, i] = self.recordings[:, i]*self.mag_scale_factor[mag_dim[i]]
                                    elif mag_dim[i] in [5, 6]:
                                        bybz_scale = np.min(self.mag_scale_factor[1:])
                                        print("scale_factor for dimension ", mag_dim[i], ": ", bybz_scale)
                                        self.recordings[:, i] = self.recordings[:, i]*bybz_scale
                                    elif mag_dim[i] in [7, 8, 9]:
                                        bxbybz_scale = np.min(self.mag_scale_factor)
                                        print("scale_factor for dimension ", mag_dim[i], ": ", bxbybz_scale)
                                        self.recordings[:, i] = self.recordings[:, i]*bxbybz_scale
                                    elif mag_dim[i] in [10]:
                                        self.recordings[:, i] = self.recordings[:, i]*self.mag_scale_factor[0]
                            elif mag_dim < 3:
                                print("All scale_factors: ", np.array(self.mag_scale_factor))
                                print("Chosen dimensions: ", mag_dim)
                                print("scale_factor for dimension ", mag_dim, ": ", self.mag_scale_factor[mag_dim])
                                self.recordings = self.recordings*self.mag_scale_factor[mag_dim]
                            elif mag_dim in [5, 6]:
                                bybz_scale = np.min(self.mag_scale_factor[1:])
                                print("scale_factor for dimension ", mag_dim, ": ", bybz_scale)
                                self.recordings = self.recordings*bybz_scale
                            elif mag_dim in [7, 8, 9]:
                                bxbybz_scale = np.min(self.mag_scale_factor)
                                print("scale_factor for dimension ", mag_dim, ": ", bxbybz_scale)
                                self.recordings = self.recordings*bxbybz_scale

                        if convert_negative == True:
                            print("Converting recordings to negative")
                            self.recordings = self.recordings*-1
                        #self.recordings = self.recordings.reshape(np.shape(self.recordings)[0], -1)

                        single_channel_sensors = np.shape(self.recordings)[-1]
                        self.recordings = self.recordings.reshape(np.shape(self.recordings)[0], -1)
                        
                        # Truncate by peak value
                        if isinstance(mag_dim, list) and 99 in mag_dim:
                            print("Truncating number of sensors - peak magnitude")
                            print("Current sensor count: ", np.shape(self.recordings)[-1])
                            print("New sensor count: ", single_channel_sensors)

                            peak_idxs = np.flip(np.argsort(np.abs(np.max(self.recordings, 0))))
                            print("Peak Idxs: ", peak_idxs)
                            print("Corresponding max vals: ", np.abs(np.max(self.recordings, 0))[peak_idxs])

                            self.recordings = self.recordings[:, peak_idxs[:single_channel_sensors]]

                        if isinstance(mag_dim, list) and 100 in mag_dim:
                            print("Shuffling data")
                            num_sensors = np.shape(self.recordings)[-1]
                            new_idxs = np.arange(num_sensors)
                            np.random.shuffle(new_idxs)
                            print("New idxs: ", new_idxs)
                            self.recordings = self.recordings[:, new_idxs]

                        # Truncate by average power
                        if isinstance(mag_dim, list) and 101 in mag_dim:
                            print("Truncating number of sensors - avg power")
                            print("Current sensor count: ", np.shape(self.recordings)[-1])
                            print("New sensor count: ", single_channel_sensors)

                            peak_idxs = np.flip(np.argsort(np.sum(self.recordings**2, axis=0)))
                            print("Peak Idxs: ", peak_idxs)
                            print("Corresponding max vals: ", np.abs(np.max(self.recordings, 0))[peak_idxs])
