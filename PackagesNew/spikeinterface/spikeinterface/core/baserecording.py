...
class BaseRecording(BaseExtractor):
    ...
    def _save(self, format='binary', magnetic_dim = -1, **save_kwargs):
        """
        ...
        """
        print("BaseRecording save called")
        if format == 'binary':
            ...
            print("Write_binary_recording called in BaseRecording")
            print("write_binary_recording self: ", self)
            write_binary_recording(self, file_paths=file_paths, dtype=dtype, magnetic_dim=magnetic_dim, **job_kwargs)
            print("Write_binary_recording finished in BaseRecording")

            from .binaryrecordingextractor import BinaryRecordingExtractor
            print("Calling BinaryRecordingExtractor")
    ...
    def set_probes(self, probe_or_probegroup, group_mode='by_probe', in_place=False):
        ...
        # Ziad Added: Generate new probe to handle combining multiple recording dimensions
        if len(inds) < len(self.get_channel_ids()) and len(self.get_channel_ids()) % len(inds) == 0:
            #print("Arr: ", arr)
            print("Columns: ", probegroup.to_dataframe().columns)
            shape = arr['contact_shapes'][0]
            print("Shape: ", shape)
            shape_params = {}
            if 'radius' in probegroup.to_dataframe().columns:
                radius = arr['radius'][0]
                print("Radius: ", radius)
                shape_params['radius'] = radius
            elif 'width' in probegroup.to_dataframe().columns:
                width = arr['width'][0]
                print("Width: ", width)
            ...
            new_x = x + x
            new_y = y + y
            if len(self.get_channel_ids()) == len(inds)*3:
                new_x = new_x + x
                new_y = new_y + y
            positions = np.zeros((len(self.get_channel_ids()), 2))
            positions[:, 0] = np.array(new_x)
            positions[:, 1] = np.array(new_y)
            probe = Probe(ndim=2, si_units='um')
            probe.set_contacts(positions=positions, shapes=shape, shape_params=shape_params)
            probe.set_device_channel_indices(np.arange(len(self.get_channel_ids()), dtype='int64'))
            new_probegroup = ProbeGroup()
            new_probegroup.add_probe(probe)
            #print("New probe: ", new_probegroup.to_numpy(complete=True))

            arr = new_probegroup.to_numpy(complete=True)
            inds = arr['device_channel_indices']
        ...
        if len(inds) < len(self.get_channel_ids()) and len(self.get_channel_ids()) % len(inds) == 0:
            ...
            positions[:, 1] = np.array(new_y)
            probe = Probe(ndim=2, si_units='um')
            probe.set_contacts(positions=positions, shapes=shape, shape_params={'radius': radius})
            probe.set_device_channel_indices(np.arange(len(self.get_channel_ids()), dtype='int64'))
            new_probegroup = ProbeGroup()
            new_probegroup.add_probe(probe)
            #print("New probe: ", new_probegroup.to_numpy(complete=True))
        ...
        arr = arr[order]
        #print("Arr in baserecording set_probes: ", arr)
        arr['device_channel_indices'] = np.arange(arr.size, dtype='int64')

        #print("Arr in baserecording set_probes: ", arr)
        #print("new_channel_ids in baserecording set_probes: ", new_channel_ids)
        #print("self.get_channel_ids() in baserecording set_probes: ", self.get_channel_ids())
