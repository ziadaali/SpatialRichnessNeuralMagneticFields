...
class BaseExtractor:
    ...
    def from_dict(d, base_folder=None):
        ...
        # print("d in from_dict of BaseExtractor", d['kwargs'])
    ...
    def save(self, magnetic_dim=-1, **kwargs):
        """
        route save_to_folder() or save_to_mem()
        """
        print("BaseExtractor save called")
        if kwargs.get('format', None) == 'memory':
            return self.save_to_memory(magnetic_dim=magnetic_dim, **kwargs)
        else:
            return self.save_to_folder(magnetic_dim=magnetic_dim, **kwargs)

    def save_to_memory(self, magnetic_dim=-1, **kwargs):
        # used only by recording at the moment
        print("save_to_memory called in Base")
        cached = self._save(magnetic_dim = magnetic_dim, **kwargs)
        self.copy_metadata(cached)
        return cached

    def save_to_folder(self, name=None, folder=None, dump_ext='json', verbose=True, magnetic_dim=-1, **save_kwargs):
        """
        ...
        print("save_to_folder called in Base")
        cached = self._save(folder=folder, verbose=verbose, magnetic_dim=magnetic_dim, **save_kwargs)
...
def _load_extractor_from_dict(dic):
    ...
    print("_load_extractor_from_dict in Base called")
    print("Cls: ", cls)
    print("kwargs: ", kwargs)
...
def load_extractor(file_or_folder_or_dict, base_folder=None):
    ...
    extractor: Recording or Sorting
        The loaded extractor object
    """
    # print("load_extractor file_or_folder_or_dict", file_or_folder_or_dict['kwargs'])
