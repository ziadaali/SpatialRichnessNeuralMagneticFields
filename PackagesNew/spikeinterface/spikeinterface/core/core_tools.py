...
def _init_binary_worker(recording, rec_memmaps_dict, dtype):
    ...
    if isinstance(recording, dict):
        ...
        print("Loading extractor in _init_binary_worker in CoreTools")
        print("Recording in _init_binary_worker: ", recording['kwargs'])
...
def write_binary_recording(recording, file_paths=None, dtype=None, add_file_extension=True,
                           verbose=False, byte_offset=0, magnetic_dim = -1, **job_kwargs):
    '''
    ...
    print("write_binary_recording in CoreTools milestone")
    print("recording in write_binary_recording: ", recording)
    ziad_rec_dict = recording.to_dict()
    if magnetic_dim != -1:
        ziad_rec_dict['kwargs']['magnetic_dim'] = magnetic_dim
    init_args = (ziad_rec_dict, rec_memmaps_dict, dtype)
    print("recording.to_dict() in write_binary_recording: ", ziad_rec_dict['kwargs'])
    #print("rec_memmaps_dict in write_binary_recording: ", rec_memmaps_dict)
