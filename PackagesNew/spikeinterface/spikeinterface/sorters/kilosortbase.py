    ...
    def _run_from_folder(cls, output_folder, params, verbose):
        ...
        if 'win' in sys.platform and sys.platform != 'darwin':
            ...
            shell_cmd = f'''
                        {disk_move}
                        cd {output_folder}
                        matlab.exe -nosplash -wait -log -r {cls.sorter_name}_master
                    '''
        else:
            shell_cmd = f'''
                        #!/bin/bash
                        cd "{output_folder}"
                        matlab -nodisplay -nosplash -wait -nodisplay -log -r {cls.sorter_name}_master
                    '''
