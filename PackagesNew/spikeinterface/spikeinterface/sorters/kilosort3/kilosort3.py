...
def modify_path_locations(path):
...
class Kilosort3Sorter(KilosortBase, BaseSorter):
    ...
    print('KILOSORT PATH: ', kilosort3_path)
    ...
    def _setup_recording(cls, recording, output_folder, params, verbose):
        ...
        print('Kilosort3 Path: ', str(Path(Kilosort3Sorter.kilosort3_path).absolute()))
        print('Output Folder: ', str(output_folder.absolute()))
        print('Channel Path: ', str((output_folder / 'kilosort3_channelmap.m').absolute()))
        print('Config Path: ', str((output_folder / 'kilosort3_config.m').absolute()))
        modify_path_locations(str((output_folder / 'kilosort3_config.m').absolute()))

        # make substitutions in txt files
        kilosort3_master_txt = kilosort3_master_txt.format(
            kilosort3_path=modify_path_locations(str(Path(Kilosort3Sorter.kilosort3_path).absolute())),
            output_folder=modify_path_locations(str(output_folder.absolute())),
            channel_path=modify_path_locations(str((output_folder / 'kilosort3_channelmap.m').absolute())),
            config_path=modify_path_locations(str((output_folder / 'kilosort3_config.m').absolute())),
        )

        kilosort3_config_txt = kilosort3_config_txt.format(
            nchan=recording.get_num_channels(),
            sample_rate=recording.get_sampling_frequency(),
            dat_file=modify_path_locations(str((output_folder / 'recording.dat').absolute())),
            nblocks=p['nblocks'],
            sig=p['sig'],
        ...
        print('Kilosort Master Text: ', kilosort3_master_txt)
