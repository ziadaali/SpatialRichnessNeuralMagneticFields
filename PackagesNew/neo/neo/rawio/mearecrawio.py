...
class MEArecRawIO(BaseRawIO):
    ...
    def _parse_header(self, magnetic_dim = -1):
        print("_parse_header magnetic_dim: ", magnetic_dim)
        try:
            import MEArec as mr
            HAVE_MEAREC = True
        except ImportError:
        ...
        print("Neo - recordings max = ", np.max(self._recordings))
        print("Recordings shape: ", self._recordings.shape)
        self._num_frames, self._num_channels = self._recordings.shape
        # if magnetic_dim != -1:
        #     mag_dim = magnetic_dim
        ...
        #     self._recordings = self._recgen.mag_recordings[:, mag_dim]
        #
        #     if self._recgen.mag_scale_factor is not None:
        #         print("Mag_dim: ", mag_dim)
        #         if isinstance(mag_dim, list):
        #             print("Multiplying by scale factor. recordings shape: ", np.shape(self._recordings))
        #             print("All scale_factors: ", np.array(self._recgen.mag_scale_factor))
        #             print("Chosen dimensions: ", mag_dim)
        #             for i in range(len(mag_dim)):
        #                 if mag_dim[i] < 3:
        #                     print("scale_factor for dimension ", mag_dim[i], ": ", self._recgen.mag_scale_factor[mag_dim[i]])
        #                     self._recordings[:, i] = self._recordings[:, i]*self._recgen.mag_scale_factor[mag_dim[i]]
        #         elif mag_dim < 3:
        #             print("All scale_factors: ", np.array(self._recgen.mag_scale_factor))
        #             print("Chosen dimensions: ", mag_dim)
        #             print("scale_factor for dimension ", mag_dim, ": ", self._recgen.mag_scale_factor[mag_dim])
        #             self._recordings = self._recordings*self._recgen.mag_scale_factor[mag_dim]
        #     if convert_negative == True:
        #         self._recordings = self._recordings*-1
        #     self._recordings = self._recordings.reshape(np.shape(self._recordings)[0], -1)
        ...
        #         convert_negative = mag_dim[1]
        #         mag_dim = mag_dim[0]
        #     print("NEO - Switching recordings in RecordingGenerator to be magnetic recordings with magnetic_dim: ", mag_dim)
        #     print("Shape self._recgen.recordings: ", np.shape(self._recgen.recordings))
        #     print("Shape self._recgen.mag_recordings: ", np.shape(self._recgen.mag_recordings))
        #     mag_dim_start = 0
        #     if isinstance(mag_dim, list):
        #         if mag_dim[0] == -1:
        #             mag_dim_start = 1
        #     self._recordings = self._recgen.mag_recordings[:, mag_dim[mag_dim_start:]]
        #     if mag_dim_start == 1:
        #         self._recordings = np.concatenate([self._recordings, self._recgen.recordings[:, None, :]], axis=1)
        #
        #     if self._recgen.mag_scale_factor is not None:
        #         print("Mag_dim: ", mag_dim)
        #         if isinstance(mag_dim, list):
        ...
        print("Recordings shape: ", np.shape(self._recordings))
