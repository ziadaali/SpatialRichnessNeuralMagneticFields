...
from MEArecGenerate.tools import (select_templates, find_overlapping_templates, get_binary_cat,
...
from MEArecGenerate.generators import SpikeTrainGenerator
...
class RecordingGenerator:
    ...
    def __init__(self, spgen=None, tempgen=None, params=None, rec_dict=None, info=None, noise_tempgen=None):
        self._verbose = False
        self._verbose_1 = False
        self._verbose_2 = False

        print("Creating RecordingGenerator")

        if rec_dict is not None and info is not None:
            print(rec_dict.keys())
            if 'magnetic_dim' in rec_dict.keys():
                print("Magnetic dim: ", rec_dict['magnetic_dim'])
            ...
            if 'mag_recordings' in rec_dict.keys():
                self.mag_recordings = rec_dict['mag_recordings']
                if 'magnetic_dim' in rec_dict.keys():
                    if rec_dict['magnetic_dim'] != -1:
                        mag_dim = rec_dict['magnetic_dim']
                        convert_negative = False
                        if type(mag_dim) == list:
                            convert_negative = mag_dim[1]
                            mag_dim = mag_dim[0]
                        print("RecGen - Switching recordings in RecordingGenerator to be magnetic recordings with magnetic_dim: ", mag_dim)
                        self.recordings = self.mag_recordings[:, mag_dim]
                        self.recordings = self.recordings.reshape(np.shape(self.recordings)[0], -1)
                        if 'mag_scale_factor' in rec_dict.keys():
                            self.mag_scale_factor = rec_dict['mag_scale_factor']
                            print("Mag Scale Factor: ", self.mag_scale_factor)
                            print("Mag_dim: ", mag_dim)
                            print("Min of mag_dim: ", np.min(mag_dim))
                            if np.min(mag_dim) < 3:
                                self.recordings = self.recordings*self.mag_scale_factor
                            if convert_negative == True:
                                self.recordings = self.recordings*-1
                else:
                    print("Magnetic_dim not in recordinggenerator keys")
            ...
            self.noise_tempgen = noise_tempgen
            self.tmp_mode = None
            self.template_ids = None
    ...
    def generate_recordings(self, tmp_mode=None, tmp_folder=None, verbose=None, template_ids=None, n_jobs=0, magnetic=False):
        """
        ...
        noise_tempgen = self.noise_tempgen
        spgen = self.spgen
        ...
        assert noise_mode in ["uncorrelated", "distance-correlated", "far-neurons", "ziad-far-neurons"], \
            "'noise_mode can be: 'uncorrelated', 'distance-correlated', 'ziad-far-neurons', or 'far-neurons'"
        ...
        #Ziad added
        if 'noise_xlim' not in temp_params.keys():
            params['templates']['noise_xlim'] = None
        noise_x_lim = params['templates']['noise_xlim']

        if 'noise_ylim' not in temp_params.keys():
            params['templates']['noise_ylim'] = None
        noise_y_lim = params['templates']['noise_ylim']

        if 'noise_zlim' not in temp_params.keys():
            params['templates']['noise_zlim'] = None
        noise_z_lim = params['templates']['noise_zlim']
        ...
        if self.tmp_mode == 'memmap':
            ...
            recordings[:] = 0
            if magnetic == True:
                tmp_path_0_mag = self.tmp_folder / (tmp_prefix + "mearec_tmp_file_mag_recordings.raw")
                mag_recordings = np.memmap(tmp_path_0_mag, shape=(n_samples, 3, n_elec), dtype=dtype, mode='w+')
                mag_recordings[:] = 0
            ...
            if not only_noise:
                ...
                spike_traces[:] = 0
                if magnetic == True:
                    tmp_path_1_mag = self.tmp_folder / (tmp_prefix + "mearec_tmp_file_mag_spike_traces.raw")
                    mag_spike_traces = np.memmap(tmp_path_1_mag, shape=(n_samples, 3, n_neurons), dtype=dtype, mode='w+')
                    mag_spike_traces[:] = 0
            ...
            tmp_templates_pad_mag_x = self.tmp_folder / (tmp_prefix + "templates_pad_mag_x.raw")
            tmp_templates_pad_mag_y = self.tmp_folder / (tmp_prefix + "templates_pad_mag_y.raw")
            tmp_templates_pad_mag_z = self.tmp_folder / (tmp_prefix + "templates_pad_mag_z.raw")

            tmp_templates_rs = self.tmp_folder / (tmp_prefix + "templates_resample.raw")
            tmp_templates_rs_mag_x = self.tmp_folder / (tmp_prefix + "templates_resample_mag_x.raw")
            tmp_templates_rs_mag_y = self.tmp_folder / (tmp_prefix + "templates_resample_mag_y.raw")
            tmp_templates_rs_mag_z = self.tmp_folder / (tmp_prefix + "templates_resample_mag_z.raw")

            tmp_templates_jit = self.tmp_folder / (tmp_prefix + "templates_jitter.raw")
            tmp_templates_jit_mag_x = self.tmp_folder / (tmp_prefix + "templates_jitter_mag_x.raw")
            tmp_templates_jit_mag_y = self.tmp_folder / (tmp_prefix + "templates_jitter_mag_y.raw")
            tmp_templates_jit_mag_z = self.tmp_folder / (tmp_prefix + "templates_jitter_mag_z.raw")
            self._to_remove_on_delete.extend([tmp_path_0, tmp_path_0_mag,
                                              tmp_path_1, tmp_path_1_mag,
                                              tmp_templates_pad, tmp_templates_pad_mag_x, tmp_templates_pad_mag_y, tmp_templates_pad_mag_z,
                                              tmp_templates_rs, tmp_templates_rs_mag_x, tmp_templates_rs_mag_y, tmp_templates_rs_mag_z,
                                              tmp_templates_jit,tmp_templates_jit_mag_x, tmp_templates_jit_mag_y, tmp_templates_jit_mag_z])
        else:
            recordings = np.zeros((n_samples, n_elec), dtype=dtype)
            spike_traces = np.zeros((n_samples, n_neurons), dtype=dtype)
            if magnetic == True:
                mag_recordings = np.zeros((n_samples, 3, n_elec), dtype=dtype)
                mag_spike_traces = np.zeros((n_samples, 3, n_neurons), dtype=dtype)
        ...
        if only_noise:
            ...
            if tempgen is not None:
                ...
                if self.template_ids is None:
                    ...
                    print("Reordered Idx Cells: ", reordered_idx_cells)
                    ...
                    if magnetic == True:
                        template_mags = np.array(mags)[reordered_idx_cells]
                    ...
                    if magnetic == True:
                        template_mags = np.array(mags)[self.template_ids]
                    self.original_templates = templates
                ...
                voltage_peaks = []
                magnetic_peaks = []
                #print("Template Shape: ", np.shape(templates))
                #print("Mag Shape: ", np.shape(template_mags))
                for i in range(len(templates)):
                    ...
                    if not drifting:
                        ...
                        if magnetic == True:
                            mag_feat = get_templates_features(mag, ['mag_peak'], dt=dt)
                    else:
                        feat = get_templates_features(tem[0], ['neg'], dt=dt)
                        if magnetic == True:
                            mag_feat = get_templates_features(mag[0], ['mag_peak'], dt=dt)
                    #print("Voltage Peak Shape: ", np.shape(feat['neg']))
                    voltage_peaks.append(-np.squeeze(feat['neg']))
                    #print("Mag Peak Shape: ", np.shape(mag_feat['mag_peak']))
                    if magnetic == True:
                        magnetic_peaks.append(np.squeeze(mag_feat['mag_peak']))
                voltage_peaks = np.array(voltage_peaks)
                if magnetic == True:
                    magnetic_peaks = np.array(magnetic_peaks)
                print("Voltage Peaks: ", np.shape(voltage_peaks))
                print("Magnetic Peaks: ", np.shape(magnetic_peaks))
                ...
                print("Non-Padded Templates Size: ", np.shape(templates))
                print("Padded Templates Size: ", np.shape(templates_pad))
                if magnetic == True:
                    mag_pad_x = pad_templates(template_mags[:, :, 0], pad_samples, drifting, dtype, verbose_2,
                                                  tmp_file=tmp_templates_pad_mag_x, parallel=parallel_templates)
                    mag_pad_y = pad_templates(template_mags[:, :, 1], pad_samples, drifting, dtype, verbose_2,
                                                  tmp_file=tmp_templates_pad_mag_y, parallel=parallel_templates)
                    mag_pad_z = pad_templates(template_mags[:, :, 2], pad_samples, drifting, dtype, verbose_2,
                                                  tmp_file=tmp_templates_pad_mag_z, parallel=parallel_templates)
                    template_mags_pad = np.stack([mag_pad_x, mag_pad_y, mag_pad_z], axis=2)
                    print("Non-Padded Magnetic Templates Size: ", np.shape(template_mags))
                    print("Padded Magnetic Templates Size: ", np.shape(template_mags_pad))
                ...
                if up != down:
                    ...
                    if magnetic == True:
                        template_mags_rs_x = resample_templates(mag_pad_x, n_resample, up, down, drifting, dtype,
                                                          verbose_2, tmp_file=tmp_templates_rs_mag_x,
                                                          parallel=parallel_templates)
                        template_mags_rs_y = resample_templates(mag_pad_y, n_resample, up, down, drifting, dtype,
                                                          verbose_2, tmp_file=tmp_templates_rs_mag_y,
                                                          parallel=parallel_templates)
                        template_mags_rs_z = resample_templates(mag_pad_z, n_resample, up, down, drifting, dtype,
                                                          verbose_2, tmp_file=tmp_templates_rs_mag_z,
                                                          parallel=parallel_templates)
                        template_mags_rs = np.stack([template_mags_rs_x, template_mags_rs_y, template_mags_rs_z], axis=2)
                        print("Magnetic Resampled Templates Shape: ", np.shape(template_mags_rs))
                    if verbose_1:
                        print('Elapsed resample time:', time.time() - t_rs)
                else:
                    templates_rs = templates_pad
                    if magnetic == True:
                        template_mags_rs = template_mags_pad

                if verbose_1:
                    print('Creating time jittering')
                jitter = 1. / fs
                t_j = time.time()
                print("Template without jitter shape: ", np.shape(templates_rs))
                ...
                print("Template with jitter shape: ", np.shape(templates))
                if magnetic == True:
                    template_mags_jitter_x = jitter_templates(template_mags_rs[:, :, 0], upsample, fs, n_jitters, jitter,
                                                            drifting, dtype, verbose_2, tmp_file=tmp_templates_jit_mag_x,
                                                            parallel=parallel_templates)
                    template_mags_jitter_y = jitter_templates(template_mags_rs[:, :, 1], upsample, fs, n_jitters, jitter,
                                                            drifting, dtype, verbose_2, tmp_file=tmp_templates_jit_mag_y,
                                                            parallel=parallel_templates)
                    template_mags_jitter_z = jitter_templates(template_mags_rs[:, :, 2], upsample, fs, n_jitters, jitter,
                                                            drifting, dtype, verbose_2, tmp_file=tmp_templates_jit_mag_z,
                                                            parallel=parallel_templates)
                    template_mags = np.stack([template_mags_jitter_x, template_mags_jitter_y, template_mags_jitter_z], axis=3)
                    print("Mag Templates with jitter shape: ", np.shape(template_mags))
                if verbose_1:
                ...
                if magnetic == True:
                    del template_mags_rs, template_mags_pad
            else:
            ...
            # ZIAD - This is being skipped but may need to be implemented later
            ...
            if magnetic == True:
                mag_args_x = (spike_idxs, pad_samples_conv, modulation, drifting, drift_mode, drifting_units,
                            template_mags[:, :, :, 0], cut_outs_samples, template_locs, velocity_vector, fast_drift_period,
                            fast_drift_min_jump, fast_drift_max_jump, t_start_drift, fs, verbose_2,
                            amp_mod, bursting_units, shape_mod, shape_stretch,
                            True, magnetic_peaks[:, 0], dtype, seed_list_conv,)
                mag_args_y = (spike_idxs, pad_samples_conv, modulation, drifting, drift_mode, drifting_units,
                            template_mags[:, :, :, 1], cut_outs_samples, template_locs, velocity_vector, fast_drift_period,
                            fast_drift_min_jump, fast_drift_max_jump, t_start_drift, fs, verbose_2,
                            amp_mod, bursting_units, shape_mod, shape_stretch,
                            True, magnetic_peaks[:, 1], dtype, seed_list_conv,)
                mag_args_z = (spike_idxs, pad_samples_conv, modulation, drifting, drift_mode, drifting_units,
                            template_mags[:, :, :, 2], cut_outs_samples, template_locs, velocity_vector, fast_drift_period,
                            fast_drift_min_jump, fast_drift_max_jump, t_start_drift, fs, verbose_2,
                            amp_mod, bursting_units, shape_mod, shape_stretch,
                            True, magnetic_peaks[:, 2], dtype, seed_list_conv,)
            ...
            if magnetic == True:
                assignment_dict_mag_x = {
                    'recordings': mag_recordings[:, 0],
                    'spike_traces': mag_spike_traces[:, 0]}
                assignment_dict_mag_y = {
                    'recordings': mag_recordings[:, 1],
                    'spike_traces': mag_spike_traces[:, 1]}
                assignment_dict_mag_z = {
                    'recordings': mag_recordings[:, 2],
                    'spike_traces': mag_spike_traces[:, 2]}

            #print("Assignment Dict: ", np.max(assignment_dict['recordings']))
            ...
            #print("Assignment Dict After Convolution: ", np.max(assignment_dict['recordings']))
            if magnetic == True:
                output_list_mag_x = run_several_chunks(chunk_convolution, chunk_indexes, fs, timestamps, mag_args_x,
                                                 self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict_mag_x)
                output_list_mag_y = run_several_chunks(chunk_convolution, chunk_indexes, fs, timestamps, mag_args_y,
                                                 self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict_mag_y)
                output_list_mag_z = run_several_chunks(chunk_convolution, chunk_indexes, fs, timestamps, mag_args_z,
                                                 self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict_mag_z)
        ...
        if noise_level == 0:
            ...
            if self.tmp_mode == 'memmap':
                ...
                if magnetic == True:
                    tmp_path_noise_mag = self.tmp_folder / (tmp_prefix + "mearec_tmp_mag_noise_file.raw")
                    additive_noise_mag = np.memmap(tmp_path_noise_mag, shape=(n_samples, 3, n_elec), dtype=dtype, mode='w+')
                    self._to_remove_on_delete.append(tmp_path_noise_mag)
            else:
                additive_noise = np.zeros((n_samples, n_elec), dtype=dtype)
                if magnetic == True:
                    additive_noise_mag = np.zeros((n_samples, 3, n_elec), dtype=dtype)

            if noise_mode == 'uncorrelated2':
                print("Ziad custom uncorrelated noise function")
                additive_noise = noise_level * np.random.randn(n_samples, n_elec)
                for i in range(3):
                    additive_noise_mag[:, i] = noise_level * np.random.randn(n_samples, n_elec)
                ...
                #print("Additive noise mag: ", additive_noise_mag)
                run_several_chunks(func, chunk_indexes, fs, timestamps, args,
                                   self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict)
                if magnetic == True:
                    assignment_dict_mag_x = {'additive_noise': additive_noise_mag[:, 0]}
                    assignment_dict_mag_y = {'additive_noise': additive_noise_mag[:, 1]}
                    assignment_dict_mag_z = {'additive_noise': additive_noise_mag[:, 2]}

                    #print("Additive noise x: ", additive_noise_mag[:, 0])
                    #print("Additive noise y: ", additive_noise_mag[:, 1])
                    #print("Additive noise z: ", additive_noise_mag[:, 2])
                    run_several_chunks(func, chunk_indexes, fs, timestamps, args,
                                       self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict_mag_x)
                    run_several_chunks(func, chunk_indexes, fs, timestamps, args,
                                       self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict_mag_y)
            ...
            elif noise_mode == "ziad-far-neurons":
                if noise_tempgen is None:
                    raise Exception("recordinggenerator.py, ziad-far-neurons requires noise_tempgen to be specified")

                if self.tmp_mode == 'memmap':
                    # file names for templates
                    tmp_templates_noise_pad = self.tmp_folder / (tmp_prefix + "templates_noise_pad.raw")
                    tmp_templates_noise_pad_magx = self.tmp_folder / (tmp_prefix + "templates_noise_pad_magx.raw")
                    tmp_templates_noise_pad_magy = self.tmp_folder / (tmp_prefix + "templates_noise_pad_magy.raw")
                    tmp_templates_noise_pad_magz = self.tmp_folder / (tmp_prefix + "templates_noise_pad_magz.raw")
                    tmp_templates_noise_rs = self.tmp_folder / (tmp_prefix + "templates_noise_resample.raw")
                    tmp_templates_noise_rs_magx = self.tmp_folder / (tmp_prefix + "templates_noise_resample_magx.raw")
                    tmp_templates_noise_rs_magy = self.tmp_folder / (tmp_prefix + "templates_noise_resample_magy.raw")
                    tmp_templates_noise_rs_magz = self.tmp_folder / (tmp_prefix + "templates_noise_resample_magz.raw")
                    self._to_remove_on_delete.extend([tmp_templates_noise_pad, tmp_templates_noise_rs,
                                                      tmp_templates_noise_pad_magx, tmp_templates_noise_pad_magy,
                                                      tmp_templates_noise_pad_magz, tmp_templates_noise_rs_magx,
                                                      tmp_templates_noise_rs_magy, tmp_templates_noise_rs_magz])
                else:
                    tmp_templates_noise_pad = None
                    tmp_templates_noise_pad_magx = None
                    tmp_templates_noise_pad_magy = None
                    tmp_templates_noise_pad_magz = None
                    tmp_templates_noise_rs = None
                    tmp_templates_noise_rs_magx = None
                    tmp_templates_noise_rs_magy = None
                    tmp_templates_noise_rs_magz = None

                noise_eaps = noise_tempgen.templates
                print("Noise EAPs Size: ", np.shape(noise_eaps))
                noise_locs = noise_tempgen.locations
                noise_rots = noise_tempgen.rotations
                if magnetic == True:
                    noise_mags = noise_tempgen.mags
                noise_celltypes = noise_tempgen.celltypes
                noise_temp_info = noise_tempgen.info
                #noise_cut_outs = noise_temp_info['params']['cut_out']

                idxs_cells, selected_cat = select_templates(noise_locs, noise_eaps, bin_cat=None, n_exc=far_neurons_n, n_inh=0,
                                                            x_lim=noise_x_lim, y_lim=noise_y_lim, z_lim=noise_z_lim, min_amp=0,
                                                            max_amp=None, min_dist=1,
                                                            verbose=False)

                idxs_cells = sorted(idxs_cells)
                templates_noise = noise_eaps[idxs_cells]
                template_noise_locs = noise_locs[idxs_cells]
                templates_noise_mags = noise_mags[idxs_cells]
                ...
                if magnetic == True:
                    templates_noise_pad_magx = pad_templates(templates_noise_mags[:, :, 0], pad_samples, drifting, dtype, verbose_2,
                                                        tmp_file=tmp_templates_noise_pad_magx, parallel=True)
                    templates_noise_pad_magy = pad_templates(templates_noise_mags[:, :, 1], pad_samples, drifting, dtype, verbose_2,
                                                        tmp_file=tmp_templates_noise_pad_magy, parallel=True)
                    templates_noise_pad_magz = pad_templates(templates_noise_mags[:, :, 2], pad_samples, drifting, dtype, verbose_2,
                                                        tmp_file=tmp_templates_noise_pad_magz, parallel=True)
                ...
                if up != down:
                    ...
                    if magnetic == True:
                        templates_noise_magx = resample_templates(templates_noise_pad_magx, n_resample, up, down,
                                                             drifting, dtype, verbose_2,
                                                             tmp_file=tmp_templates_noise_rs_magx)
                        templates_noise_magy = resample_templates(templates_noise_pad_magy, n_resample, up, down,
                                                             drifting, dtype, verbose_2,
                                                             tmp_file=tmp_templates_noise_rs_magy)
                        templates_noise_magz = resample_templates(templates_noise_pad_magz, n_resample, up, down,
                                                             drifting, dtype, verbose_2,
                                                             tmp_file=tmp_templates_noise_rs_magz)
                    if verbose_1:
                        print('Elapsed resample time:', time.time() - t_rs)
                else:
                    templates_noise = templates_noise_pad
                    if magnetic == True:
                        templates_noise_magx = templates_noise_pad_magx
                        templates_noise_magy = templates_noise_pad_magy
                        templates_noise_magz = templates_noise_pad_magz
                ...
                del templates_noise_pad, templates_noise_pad_magx, templates_noise_pad_magy, templates_noise_pad_magz

                # create noisy spiketrains
                if verbose_1:
                    print('Generating noisy spike trains')
                noisy_spiketrains_params = params['spiketrains']
                noisy_spiketrains_params['n_exc'] = far_neurons_n
                noisy_spiketrains_params['n_inh'] = 0
                ...
                if magnetic == True:
                    templates_noise_magx = templates_noise_magx.reshape((templates_noise_magx.shape[0], 1,
                                                                         templates_noise_magx.shape[1], templates_noise_magx.shape[2]))
                    templates_noise_magy = templates_noise_magy.reshape((templates_noise_magy.shape[0], 1,
                                                                         templates_noise_magy.shape[1], templates_noise_magy.shape[2]))
                    templates_noise_magz = templates_noise_magz.reshape((templates_noise_magz.shape[0], 1,
                                                                         templates_noise_magz.shape[1], templates_noise_magz.shape[2]))
                ...
                if magnetic == True:
                    args_magx = (spike_idxs_noise, 0, 'none', False, None, None, templates_noise_magx,
                            cut_outs_samples, template_noise_locs, None, None, None, None, None, None,
                            verbose_2, None, None, False, None, False, None, dtype, seed_list_noise,)
                    args_magy = (spike_idxs_noise, 0, 'none', False, None, None, templates_noise_magy,
                            cut_outs_samples, template_noise_locs, None, None, None, None, None, None,
                            verbose_2, None, None, False, None, False, None, dtype, seed_list_noise,)
                    args_magz = (spike_idxs_noise, 0, 'none', False, None, None, templates_noise_magz,
                            cut_outs_samples, template_noise_locs, None, None, None, None, None, None,
                            verbose_2, None, None, False, None, False, None, dtype, seed_list_noise,)

                    assignment_dict_mag_x = {'recordings': additive_noise_mag[:, 0]}
                    assignment_dict_mag_y = {'recordings': additive_noise_mag[:, 1]}
                    assignment_dict_mag_z = {'recordings': additive_noise_mag[:, 2]}

                run_several_chunks(chunk_convolution, chunk_indexes, fs, timestamps, args,
                                   self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict)
                if magnetic == True:
                    run_several_chunks(chunk_convolution, chunk_indexes, fs, timestamps, args_magx,
                                       self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict_mag_x)
                    run_several_chunks(chunk_convolution, chunk_indexes, fs, timestamps, args_magy,
                                       self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict_mag_y)
                    run_several_chunks(chunk_convolution, chunk_indexes, fs, timestamps, args_magz,
                                       self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict_mag_z)
                ...
                for i, m in enumerate(np.mean(additive_noise, axis=0)):
                    additive_noise[:, i] -= m
                if magnetic == True:
                    for i, m in enumerate(np.mean(additive_noise_mag[:, 0], axis=0)):
                        additive_noise_mag[:, 0][:, i] -= m
                    for i, m in enumerate(np.mean(additive_noise_mag[:, 1], axis=0)):
                        additive_noise_mag[:, 1][:, i] -= m
                    for i, m in enumerate(np.mean(additive_noise_mag[:, 2], axis=0)):
                        additive_noise_mag[:, 2][:, i] -= m
            ...
            if magnetic == True:
                # Calculate the snr of the peak recorded signal and scale mag_recordings to have the same SNR for the peaks
                print("Magnetic Shape: ", np.shape(mag_recordings))
                peak_elec = np.unravel_index(np.argmax(np.abs(recordings)), np.shape(recordings))[1]
                print("Peak Electric Electrode: ", peak_elec)
                #peak_mag_elec = np.unravel_index(np.argmax(np.abs(mag_recordings)), np.shape(mag_recordings))[2]
                #print("Peak Magnetic Electrode: ", peak_mag_elec)
                elec_power = np.sum(recordings**2, axis=0)/len(recordings)
                peak_elec_power_idx = np.argmax(elec_power)
                avg_elec_power = np.mean(elec_power)
                print("Peak Elec Power Index: ", peak_elec_power_idx)
                print("Peak Elec Power: ", elec_power[peak_elec_power_idx])
                print("Average Elec Power: ", avg_elec_power)

                peak_elec_sigs = np.max(np.abs(recordings), axis=0)
                peak_elec_sig_idx = np.argmax(peak_elec_sigs)
                print(f"Peak Elec Signal Index: ", peak_elec_sig_idx)
                print(f"Peak Elec Signal: ", peak_elec_sigs[peak_elec_sig_idx])

                mag_scale_factor = []
                for mag_dim in [0, 1, 2]:
                    mag_recordings_single_dim = mag_recordings[:, mag_dim]

                    if self.params['recordings']['mag_noise'] == 'equalize_peak_sig':
                        peak_mag_sigs = np.max(mag_recordings_single_dim, axis=0)
                        peak_mag_sig_idx = np.argmax(peak_mag_sigs)
                        print(f"Peak Mag Signal Index, dim {mag_dim}: ", peak_mag_sig_idx)
                        print(f"Peak Mag Signal, dim {mag_dim}: ", peak_mag_sigs[peak_mag_sig_idx])

                        mag_scale_factor_dim = peak_elec_sigs[peak_elec_sig_idx]/peak_mag_sigs[peak_mag_sig_idx]
                        print(f"Mag Scale Factor, dim = {mag_dim}", mag_scale_factor_dim)

                    elif self.params['recordings']['mag_noise'] == 'equalize_avg_power':
                        mag_power = np.sum(mag_recordings_single_dim**2, axis=0)/len(mag_recordings_single_dim)
                        avg_mag_power = np.mean(mag_power)
                        print("Average Mag Power: ", avg_mag_power)

                        mag_scale_factor_dim = np.sqrt(avg_elec_power/avg_mag_power)
                        print(f"Mag Scale Factor, dim = {mag_dim}", mag_scale_factor_dim)

                    else: #equalize_peak_power
                        peak_mag_power = np.sum(mag_recordings_single_dim**2, axis=0)/len(mag_recordings_single_dim)
                        peak_mag_power_idx = np.argmax(peak_mag_power)
                        print(f"Peak Mag Power Index, dim {mag_dim}: ", peak_mag_power_idx)
                        print(f"Peak Mag Power, dim {mag_dim}: ", peak_mag_power[peak_mag_power_idx])

                        mag_scale_factor_dim = np.sqrt(elec_power[peak_elec_power_idx]/peak_mag_power[peak_mag_power_idx])

                    mag_scale_factor.append(mag_scale_factor_dim)

                # Create array to hold [1, 2] and [0, 1, 2] mag_recordings scaled by same parameter
                mag_recordings_group = np.zeros((np.shape(mag_recordings)[0], 5, np.shape(mag_recordings)[2]))
                mag_recordings_group[:, 0:2] = np.copy(mag_recordings[:, 1:3])
                mag_recordings_group[:, 2:5] = np.copy(mag_recordings[:, 0:3])

                # Scale By and Bz fields by the smaller of their two scale factors
                bybz_scale = np.min(mag_scale_factor[1:])
                for i in range(2):
                    mag_recordings_group[:, i] *= bybz_scale
                    mag_recordings_group[:, i] += additive_noise_mag[:, i+1]
                    mag_recordings_group[:, i] /= bybz_scale

                bxbybz_scale = np.min(mag_scale_factor)
                for i in range(3):
                    mag_recordings_group[:, i+2] *= bxbybz_scale
                    mag_recordings_group[:, i+2] += additive_noise_mag[:, i]
                    mag_recordings_group[:, i+2] /= bxbybz_scale

                for mag_dim in [0, 1, 2]:
                    # If noise mode is for far neurons, additive noise has already been scaled because it is a function of neurons
                    if noise_mode == "ziad-far-neurons":
                        mag_recordings[:, mag_dim] += additive_noise_mag[:, mag_dim]
                    else:
                        mag_recordings[:, mag_dim] *= mag_scale_factor[mag_dim]
                        mag_recordings[:, mag_dim] += additive_noise_mag[:, mag_dim]
                        mag_recordings[:, mag_dim] /= mag_scale_factor[mag_dim]

                print("Mag Scale Factors: ", mag_scale_factor, "Min: ", np.min(mag_scale_factor))
                print("Bx: ", np.sum(mag_recordings[:, 0]**2))
                print("By: ", np.sum(mag_recordings[:, 1]**2))
                print("Bz: ", np.sum(mag_recordings[:, 2]**2))
                print("By 1: ", np.sum(mag_recordings_group[:, 0]**2))
                print("Bz 1: ", np.sum(mag_recordings_group[:, 1]**2))
                print("Bx 2: ", np.sum(mag_recordings_group[:, 2]**2))
                print("By 2: ", np.sum(mag_recordings_group[:, 3]**2))
                print("Bz 2: ", np.sum(mag_recordings_group[:, 4]**2))
        ...
        if filter:
            ...
            run_several_chunks(chunk_apply_filter, chunk_indexes, fs, timestamps, args,
                               self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict)
            if magnetic == True:
                args_mag_x = (mag_recordings[:, 0], pad_samples_filt, cutoff, order, fs, dtype,)
                args_mag_y = (mag_recordings[:, 1], pad_samples_filt, cutoff, order, fs, dtype,)
                args_mag_z = (mag_recordings[:, 2], pad_samples_filt, cutoff, order, fs, dtype,)

                args_mag_bybz_y = (mag_recordings_group[:, 0], pad_samples_filt, cutoff, order, fs, dtype,)
                args_mag_bybz_z = (mag_recordings_group[:, 1], pad_samples_filt, cutoff, order, fs, dtype,)
                args_mag_bxbybz_x = (mag_recordings_group[:, 2], pad_samples_filt, cutoff, order, fs, dtype,)
                args_mag_bxbybz_y = (mag_recordings_group[:, 3], pad_samples_filt, cutoff, order, fs, dtype,)
                args_mag_bxbybz_z = (mag_recordings_group[:, 4], pad_samples_filt, cutoff, order, fs, dtype,)

                assignment_dict_mag_x = {'filtered_chunk': mag_recordings[:, 0]}
                assignment_dict_mag_y = {'filtered_chunk': mag_recordings[:, 1]}
                assignment_dict_mag_z = {'filtered_chunk': mag_recordings[:, 2]}

                assignment_dict_mag_bybz_y = {'filtered_chunk': mag_recordings_group[:, 0]}
                assignment_dict_mag_bybz_z = {'filtered_chunk': mag_recordings_group[:, 1]}
                assignment_dict_mag_bxbybz_x = {'filtered_chunk': mag_recordings_group[:, 2]}
                assignment_dict_mag_bxbybz_y = {'filtered_chunk': mag_recordings_group[:, 3]}
                assignment_dict_mag_bxbybz_z = {'filtered_chunk': mag_recordings_group[:, 4]}

                run_several_chunks(chunk_apply_filter, chunk_indexes, fs, timestamps, args_mag_x,
                                   self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict_mag_x)
                run_several_chunks(chunk_apply_filter, chunk_indexes, fs, timestamps, args_mag_y,
                                   self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict_mag_y)
                run_several_chunks(chunk_apply_filter, chunk_indexes, fs, timestamps, args_mag_z,
                                   self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict_mag_z)

                run_several_chunks(chunk_apply_filter, chunk_indexes, fs, timestamps, args_mag_bybz_y,
                                   self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict_mag_bybz_y)
                run_several_chunks(chunk_apply_filter, chunk_indexes, fs, timestamps, args_mag_bybz_z,
                                   self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict_mag_bybz_z)
                run_several_chunks(chunk_apply_filter, chunk_indexes, fs, timestamps, args_mag_bxbybz_x,
                                   self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict_mag_bxbybz_x)
                run_several_chunks(chunk_apply_filter, chunk_indexes, fs, timestamps, args_mag_bxbybz_y,
                                   self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict_mag_bxbybz_y)
                run_several_chunks(chunk_apply_filter, chunk_indexes, fs, timestamps, args_mag_bxbybz_z,
                                   self.n_jobs, self.tmp_mode, self.tmp_folder, assignment_dict_mag_bxbybz_z)


        # Generate [Ex, Ey] from V data
        if magnetic == True:

            #Ziad - add extra fields
            new_mag = np.zeros((np.shape(mag_recordings)[0], 10, np.shape(mag_recordings)[2]))
            new_mag[:, 0:3, :] = mag_recordings
            new_mag[:, 5:10, :] = mag_recordings_group

            Ex = np.zeros(np.shape(recordings))
            Ey = np.zeros(np.shape(recordings))
            print("Electrode Recordings Shape: ", np.shape(recordings))
            elecs_row = int(np.sqrt(np.shape(recordings)[-1]))
            print("Num electrodes in a row/col: ", elecs_row)
            if elecs_row**2 == np.shape(recordings)[-1]:

                # Generate Ey
                for i in range(np.shape(recordings)[-1]):
                    if (i+1) % elecs_row != 0:
                        Ey[:, i] = recordings[:, i+1] - recordings[:, i]

                # Generate Ex
                for i in range(np.shape(recordings)[-1]-elecs_row):
                    Ex[:, i] = recordings[:, i+elecs_row] - recordings[:, i]

                new_mag[:, 3, :] = Ex
                new_mag[:, 4, :] = Ey

            else:
                print("Skipping Ex and Ey - grid is not square")

            mag_recordings = new_mag
        ...
        self.info = params
        if magnetic == True:
            self.mag_recordings = mag_recordings
            self.mag_spike_traces = mag_spike_traces
