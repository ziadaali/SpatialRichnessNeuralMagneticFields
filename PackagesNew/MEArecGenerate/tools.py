...
def load_templates(templates, return_h5_objects=True, verbose=False, check_suffix=True):
    ...
    """
    from MEArecGenerate import TemplateGenerator
    if verbose:
    ...
    if (templates.suffix in ['.h5', '.hdf5']) or (not check_suffix):
        ...
        if 'mags' in f.keys():
            if return_h5_objects:
                temp_dict['mags'] = f.get('mags')
            else:
                temp_dict['mags'] = np.array(f.get('mags'))
    else:
...
def load_recordings(recordings, return_h5_objects=True,
    ...
    """
    from MEArecGenerate import RecordingGenerator
    if verbose:
    ...
    if (recordings.suffix in ['.h5', '.hdf5']) or (not check_suffix):
        ...
        if f.get('recordings') is not None and 'recordings' in load:
            ...
            if f.get('mag_recordings') is not None:
                print("Returning mag_recordings in MEArec tools file")
                if return_h5_objects:
                    rec_dict['mag_recordings'] = f.get('mag_recordings')
                else:
                    arr = np.array(f.get('mag_recordings'))
                    rec_dict['mag_recordings'] = arr
            if f.get('mag_scale_factor') is not None:
                if return_h5_objects:
                    rec_dict['mag_scale_factor'] = f.get('mag_scale_factor')
                else:
                    arr = np.array(f.get('mag_scale_factor'))
                    rec_dict['mag_scale_factor'] = arr
...
def save_template_generator(tempgen, magnetic = False, filename=None, verbose=True):
    """
    ...
    with h5py.File(filename, 'w') as f:
        ...
        if magnetic == True:
            if len(tempgen.mags) > 0:
                f.create_dataset('mags', data=tempgen.mags)
    if verbose:
...
def save_recording_generator(recgen, filename=None, verbose=False):
    ...
    with h5py.File(filename, 'w') as f:
        ...
        if hasattr(recgen, 'mag_recordings'):
            print("Has Attribute mag_recordings")
            f.create_dataset('mag_recordings', data=recgen.mag_recordings)
        if len(recgen.mag_recordings) > 0:
            print("Length mag_recordings > 0")
        if hasattr(recgen, 'mag_scale_factor'):
            f.create_dataset('mag_scale_factor', data=recgen.mag_scale_factor)
...
def recursively_save_dict_contents_to_group(h5file, path, dic):
    ...
    for key, item in dic.items():
        if isinstance(item, (int, float, np.integer, np.float64, str, bytes, np.bool_)):
...
def get_templates_features(templates, feat_list, dt=None, templates_times=None, threshold_detect=0):
    ...
    elif len(templates.shape) == 3 and 'mag_peak' in feat_list:
        templates = np.reshape(templates, [1, templates.shape[0], templates.shape[1], templates.shape[2]])
    if len(templates.shape) != 3 and 'mag_peak' not in feat_list:
    ...
    mag_peak_x = np.zeros((templates.shape[0], templates.shape[1]))
    mag_peak_y = np.zeros((templates.shape[0], templates.shape[1]))
    mag_peak_z = np.zeros((templates.shape[0], templates.shape[1]))
    ...
    for i in range(templates.shape[0]):
        ...
        #ZIAD NOTES - Creates array of len # electrodes corresponding to argmin index for each electrode for a single cell
        if 'mag_peak' not in feat_list:
            ...
            # for na and rep
            #ZIAD NOTES - get electrode id and time series index of overall minimum electrode value for a single cell
            ...
            _, max_idx_mag_x = np.unravel_index(abs(templates[i, 0]).argmax(), templates[i, 0].shape)
            _, max_idx_mag_y = np.unravel_index(abs(templates[i, 1]).argmax(), templates[i, 1].shape)
            _, max_idx_mag_z = np.unravel_index(abs(templates[i, 2]).argmax(), templates[i, 2].shape)
            mag_peak_x[i, :] = templates[i, :, 0, max_idx_mag_x]
            mag_peak_y[i, :] = templates[i, :, 1, max_idx_mag_y]
            mag_peak_z[i, :] = templates[i, :, 2, max_idx_mag_z]
    ...
    if 'mag_peak' in feat_list:
        mag_peak = np.zeros((3, np.shape(mag_peak_x)[0], np.shape(mag_peak_x)[1]))
        mag_peak[0] = np.array(mag_peak_x)
        mag_peak[1] = np.array(mag_peak_y)
        mag_peak[2] = np.array(mag_peak_z)
        features.update({'mag_peak': mag_peak})
...
def select_templates(loc, templates, bin_cat, n_exc, n_inh, min_dist=25, x_lim=None, y_lim=None, z_lim=None,
                     ...
                     n_overlap_pairs=None, overlap_threshold=0.8, verbose=True):
    """
                    ...
                    if np.any(dist < min_dist):
                        ...
                        if not drifting:
                            if is_position_within_boundaries(loc[id_cell], x_lim, y_lim, z_lim) \
                                ...
                                if verbose:
                                    print('Amplitude or boundary violation 1', amp, loc[id_cell], iter)
                        else:
                            # drifting
                            if is_position_within_boundaries(loc[id_cell, 0], x_lim, y_lim, z_lim) \
                                ...
                                if verbose:
                                    print('Amplitude or boundary violation 2', amp, loc[id_cell, 0], iter)
                    ...
                    if np.any(dist < min_dist):
                        ...
                        if not drifting:
                            if is_position_within_boundaries(loc[id_cell], x_lim, y_lim, z_lim) \
                                ...
                                if verbose:
                                    print('Amplitude or boundary violation 3', amp, loc[id_cell], iter)
                        else:
                            # drifting
                            if is_position_within_boundaries(loc[id_cell, 0], x_lim, y_lim, z_lim) \
                                    and min_amp < amp < max_amp:
                                ...
                                if verbose:
                                    print('Amplitude or boundary violation 4', amp, loc[id_cell, 0], iter)
                    if placed:
                        n_sel_inh += 1
            ...
            if np.any(dist < min_dist):
                ...
                if not drifting:
                    if is_position_within_boundaries(loc[id_cell], x_lim, y_lim, z_lim) and min_amp < amp < max_amp:
                        ...
                        if verbose:
                            print('Amplitude or boundary violation 5', amp, loc[id_cell], iter)
                            print("Boundaries: [", x_lim, y_lim, z_lim, "]")
                            print("Min Amp, Max Amp: ", min_amp, max_amp)
                else:
                    # drifting
                    if is_position_within_boundaries(loc[id_cell, 0], x_lim, y_lim, z_lim) and min_amp < amp < max_amp:
                        ...
                        if verbose:
                            print('Amplitude or boundary violation 6', amp, loc[id_cell, 0], iter)
            if placed:
...
def plot_templates(gen, template_ids=None, single_jitter=True, ax=None, single_axes=False, max_templates=None,
                   drifting=False, cmap=None, ncols=6, magnetic = False, dim = 0, **kwargs):
    """
    Plot templates.
    ...
    if magnetic == True:
        templates = gen.mags[:, :, dim, :]*1e12 # Convert to pT
