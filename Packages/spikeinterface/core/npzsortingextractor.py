...
class NpzSortingExtractor(BaseSorting):
    ...
    def write_sorting(sorting, save_path):
        #print("Starting write_sorting npzsortingextractor")
        d = {}
        units_ids = np.array(sorting.get_unit_ids())
        d['unit_ids'] = units_ids
        #print("Write_sorting unit ids: ", units_ids)
        ...
        #print("Write_sorting num segments: ", sorting.get_num_segments())
        ...
        for seg_index in range(sorting.get_num_segments()):
            ...
            for unit_id in units_ids:
                ...
                #if unit_id == 36:
                #    print("Unit_id ", unit_id, " indices: ", sp_ind)
                #    print("As type int64: ", sp_ind.astype('int64'))
                #    print("Spike_indexes last element: ", spike_indexes[-1])

                # spike_labels.append(np.ones(sp_ind.size, dtype='int64')*unit_id)
                spike_labels.append(np.array([unit_id] * sp_ind.size))

            #print("Write_sorting reached")
            #print("Write_sorting spike_indexes: ", np.shape(spike_indexes))
            #print("Write_sorting spike_labels: ", np.shape(spike_labels))
            #print("Write_sorting length: ", len(spike_indexes))

            #for i in range(len(spike_indexes)):
                #print("Shape ", i, ": ", np.shape(spike_indexes[i]))
                #if len(spike_indexes[i]) < 3:
                #    print("Short spike_index at ", i, ", Data: ", spike_indexes[i])
            # order times
