...
class MEArecRawIO(BaseRawIO):
    ...
    def _parse_header(self, magnetic_dim = -1):
        ...
        if magnetic_dim != -1:
            print("Magnetic Data shape: ", np.shape(self._recgen.mag_recordings))
            print("Mag_scale_factor: ", self._recgen.mag_scale_factor)
            ...
            if np.min(mag_dim) < 3:   
                self._recordings = self._recordings*self._recgen.mag_scale_factor
            if convert_negative == True:
                self._recordings = self._recordings*-1
            print("Magnetic recordings max = ", np.max(self._recordings))
